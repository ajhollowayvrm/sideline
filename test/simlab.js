...
(function () {
  ...
  for (let s = 0; s < 40; s++) {
    ...
    hit('give', /turnovers now for/); hit('sackAg', /has given up/);
    hit('flagP', /flag on/); hit('flagT', /flags now on/);
    sm = sm.concat(lines);
  }
  ...
  check('Phase 55.1: every UPSIDE category is reachable — yardage, team total, sacks, a drive one man owned',
    seen.rush > 0 && seen.team > 0 && seen.sack > 0 && seen.drive > 0,
    `rush ${seen.rush} · team ${seen.team} · sack ${seen.sack} · drive ${seen.drive} over 40 games`);
  // The other half of the game. Each has its own counter or box key, and each has to actually fire —
  // a downside milestone nobody ever crosses is a line of dead code, not a feature.
